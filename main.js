...
app.use(compression());
//함수를 호출하면 미들웨어를 리턴하도록 되어있고 app.use를 통해 장착된다.
//gzip으로 인코딩하고 해제하면서 사용한다.
//압축, 해제보다 네트워크 전송비용이 더 많이 들어서 이게 훨 좋다.

app.use(bodyparser.urlencoded({ extended: false }));
//use의 안에 미들웨어가 들어오게 된다.
//내부적으로 사용자가 전송한 post데이터를 내부적으로 분석해서
//내 app에서 사용할 때, 내 앱의 콜백을 호출하도록 약속되어있다.
...
app.post("/create_process", function(request, response) {
  //body-parser라는 미들웨어를 통해 간단한게 처리된다.
